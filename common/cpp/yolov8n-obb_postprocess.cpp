...
std::vector<Head> BuildFp32Heads(const std::vector<std::vector<uint8_t>>& outputs, size_t channels) {
  ...
  for (const auto& out : outputs) {
    if (out.size() % sizeof(float) != 0) throw std::runtime_error("OBB decode: non-fp32 tensor");
    const size_t count = out.size() / sizeof(float);
    if (count % channels != 0) throw std::runtime_error("OBB decode: invalid channels");
    const size_t hw = count / channels;
    const size_t side = static_cast<size_t>(std::sqrt(static_cast<double>(hw)));
    if (side * side != hw) throw std::runtime_error("OBB decode: non-square head");
...
std::vector<Head> BuildInt8Heads(const std::vector<std::vector<uint8_t>>& outputs,
  ...
  if (outputs.size() != q.size()) throw std::runtime_error("OBB decode: quant size mismatch");
  std::vector<Head> heads;
  heads.reserve(outputs.size());
...
std::vector<OrientedDetection> RotatedNms(std::vector<OrientedDetection> detections, float iou_threshold,
                                          size_t max_det) {
...
std::vector<OrientedDetection> DecodeImpl(const std::vector<Head>& angle_heads,
  ...
  if (angle_heads.size() != cls_heads.size() || cls_heads.size() != bbox_heads.size() ||
      angle_heads.empty()) {
    throw std::runtime_error("OBB decode: head count mismatch");
  }
  ...
  for (size_t head_idx = 0; head_idx < bbox_heads.size(); ++head_idx) {
    ...
    if (a.channels != 1 || c.channels != static_cast<size_t>(class_count) ||
        b.channels != static_cast<size_t>(reg_max * 4) || a.h != c.h || c.h != b.h || a.w != c.w ||
        c.w != b.w) {
      throw std::runtime_error("OBB decode: head shape mismatch");
    }
    const int stride = input_size / static_cast<int>(b.h);
    if (!(stride == 8 || stride == 16 || stride == 32)) {
      throw std::runtime_error("OBB decode: unsupported stride");
    }
    for (size_t y = 0; y < b.h; ++y) {
      for (size_t x = 0; x < b.w; ++x) {
        ...
        const float theta = (Sigmoid(raw_angle) - 0.25f) * kPi;
...
std::vector<OrientedDetection> DecodeYoloV8ObbMultiOutput(
  ...
  return DecodeImpl(angle_heads, cls_heads, bbox_heads, info, image_width, image_height,
                    confidence_threshold, iou_threshold, class_count, reg_max, input_size);
}
...
std::vector<OrientedDetection> DecodeYoloV8ObbMultiOutputInt8(
    ...
    int image_height, float confidence_threshold, float iou_threshold, int class_count, int reg_max,
    int input_size) {

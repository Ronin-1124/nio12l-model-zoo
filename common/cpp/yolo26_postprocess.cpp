...
std::vector<Head> BuildFp32Heads(const std::vector<std::vector<uint8_t>>& outputs,
  ...
  for (const auto& out : outputs) {
    if (out.size() % sizeof(float) != 0) throw std::runtime_error("YOLO26 decode: non-fp32 output");
    const size_t count = out.size() / sizeof(float);
    if (count % expected_channels != 0) {
      throw std::runtime_error("YOLO26 decode: channel mismatch");
    }
    const size_t hw = count / expected_channels;
    const size_t side = static_cast<size_t>(std::sqrt(static_cast<double>(hw)));
    if (side * side != hw) throw std::runtime_error("YOLO26 decode: non-square grid");
...
std::vector<Head> BuildInt8Heads(const std::vector<std::vector<uint8_t>>& outputs,
                                 const std::vector<Yolo26QuantParam>& quant_params,
                                 size_t expected_channels) {
  if (outputs.size() != quant_params.size()) {
    throw std::runtime_error("YOLO26 int8 decode: quant size mismatch");
  }
  std::vector<Head> heads;
  heads.reserve(outputs.size());
  for (size_t i = 0; i < outputs.size(); ++i) {
    ...
    if (count % expected_channels != 0) {
      throw std::runtime_error("YOLO26 int8 decode: channel mismatch");
    }
    const size_t hw = count / expected_channels;
    const size_t side = static_cast<size_t>(std::sqrt(static_cast<double>(hw)));
    if (side * side != hw) throw std::runtime_error("YOLO26 int8 decode: non-square grid");
...
std::vector<Detection> DecodeImpl(const std::vector<Head>& bbox_heads,
  ...
  if (bbox_heads.size() != cls_heads.size() || bbox_heads.empty()) {
    throw std::runtime_error("YOLO26 decode: bbox/cls head count mismatch");
  }
  ...
  for (size_t head_idx = 0; head_idx < bbox_heads.size(); ++head_idx) {
    ...
    if (b.channels != 4 || c.channels != static_cast<size_t>(class_count) || b.h != c.h ||
        b.w != c.w) {
      throw std::runtime_error("YOLO26 decode: head shape mismatch");
    }
    const int stride = input_size / static_cast<int>(b.h);
    if (!(stride == 8 || stride == 16 || stride == 32)) {
      throw std::runtime_error("YOLO26 decode: unsupported stride");
    }
...
std::vector<Detection> DecodeYolo26MultiOutput(
  ...
  return DecodeImpl(bbox_heads, cls_heads, info, image_width, image_height, confidence_threshold,
                    iou_threshold, class_count, input_size);
}
...
std::vector<Detection> DecodeYolo26MultiOutputInt8(
    ...
    int image_width, int image_height, float confidence_threshold, float iou_threshold,
    int class_count, int input_size) {
  const auto bbox_heads = BuildInt8Heads(bbox_outputs, bbox_quant_params, 4);
  const auto cls_heads = BuildInt8Heads(cls_outputs, cls_quant_params, static_cast<size_t>(class_count));
  return DecodeImpl(bbox_heads, cls_heads, info, image_width, image_height, confidence_threshold,
                    iou_threshold, class_count, input_size);
}

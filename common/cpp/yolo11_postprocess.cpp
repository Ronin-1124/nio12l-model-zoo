...
float HeadValue(const Head& head, size_t c, size_t y, size_t x) {
  ...
  if (!head.int8) {
    return reinterpret_cast<const float*>(head.data)[idx];
  }
...
std::vector<Detection> DecodeImpl(const std::vector<Head>& bbox_heads,
  ...
  if (bbox_heads.size() != cls_heads.size() || bbox_heads.empty()) {
    throw std::runtime_error("YOLO11 decode: bbox/cls head count mismatch");
  }
  ...
  for (size_t head_idx = 0; head_idx < bbox_heads.size(); ++head_idx) {
    ...
    if (b.h != b.w || c.h != c.w || b.h != c.h) {
      throw std::runtime_error("YOLO11 decode: head spatial mismatch");
    }
    if (b.channels != bbox_channels || c.channels != static_cast<size_t>(class_count)) {
      throw std::runtime_error("YOLO11 decode: head channel mismatch");
    }
    const int stride = input_size / static_cast<int>(b.h);
    if (!(stride == 8 || stride == 16 || stride == 32)) {
      throw std::runtime_error("YOLO11 decode: unsupported stride");
    }
...
std::vector<Head> BuildFp32Heads(const std::vector<std::vector<uint8_t>>& outputs,
  ...
  for (const auto& out : outputs) {
    if (out.size() % sizeof(float) != 0) {
      throw std::runtime_error("YOLO11 decode: output is not float32");
    }
    const size_t count = out.size() / sizeof(float);
    if (count % expected_channels != 0) {
      throw std::runtime_error("YOLO11 decode: output count not divisible by channels");
    }
    const size_t hw = count / expected_channels;
    const size_t side = static_cast<size_t>(std::sqrt(static_cast<double>(hw)));
    if (side * side != hw) {
      throw std::runtime_error("YOLO11 decode: output grid not square");
    }
...
std::vector<Head> BuildInt8Heads(const std::vector<std::vector<uint8_t>>& outputs,
                                 const std::vector<Yolo11QuantParam>& quant_params,
                                 size_t expected_channels) {
  if (outputs.size() != quant_params.size()) {
    throw std::runtime_error("YOLO11 int8 decode: outputs and quant mismatch");
  }
  std::vector<Head> heads;
  heads.reserve(outputs.size());
  for (size_t i = 0; i < outputs.size(); ++i) {
    ...
    if (q.scale <= 0.0f) {
      throw std::runtime_error("YOLO11 int8 decode: quant scale must be positive");
    }
    const size_t count = out.size();
    if (count % expected_channels != 0) {
      throw std::runtime_error("YOLO11 int8 decode: output count not divisible by channels");
    }
    const size_t hw = count / expected_channels;
    const size_t side = static_cast<size_t>(std::sqrt(static_cast<double>(hw)));
    if (side * side != hw) {
      throw std::runtime_error("YOLO11 int8 decode: output grid not square");
    }
    heads.push_back(Head{out.data(), expected_channels, side, side, true, q.scale, q.zero_point});
  }
  return heads;
}

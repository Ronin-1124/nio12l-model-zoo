...
#include "common/cpp/yolov10_postprocess.h"
#include "examples/yolov10n/yolov10n_config.h"
...
struct Options {
  ...
  std::string output_dir = mtkdemo::yolov10n::kDefaultOutputDir;
  size_t threads = 1;
  size_t backlog = 1;
...
Options ParseArgs(int argc, char** argv) {
  ...
  if (!options.model_overridden) {
    options.model_path = options.use_fp32 ? mtkdemo::yolov10n::kDefaultFp32ModelPath
                                          : mtkdemo::yolov10n::kDefaultInt8ModelPath;
  }
  if (options.image_paths.empty()) {
    options.image_paths.assign(mtkdemo::yolov10n::kDefaultImagePaths.begin(),
                               mtkdemo::yolov10n::kDefaultImagePaths.end());
  }
  return options;
}
...
int main(int argc, char** argv) {
  try {
    ...
    if (runtime.InputCount() != 1 || runtime.OutputCount() != 6) {
      throw std::runtime_error("Current demo expects one-input six-output YOLOv10n model");
    }
    const size_t input_size = runtime.InputSize(0);
    if (input_size == static_cast<size_t>(mtkdemo::yolov10n::kInputWidth) *
                          static_cast<size_t>(mtkdemo::yolov10n::kInputHeight) * 3U) {
      use_int8_io = true;
    } else if (input_size == static_cast<size_t>(mtkdemo::yolov10n::kInputWidth) *
                                 static_cast<size_t>(mtkdemo::yolov10n::kInputHeight) * 3U *
                                 sizeof(float)) {
      use_int8_io = false;
    } else {
      throw std::runtime_error("Unsupported model input size for YOLOv10n demo: " +
    ...
    for (const std::string& image_path : options.image_paths) {
      ...
      if (use_int8_io) {
        const std::vector<int8_t> input_tensor = mtkdemo::PreprocessToInt8RgbNchw(
            image, mtkdemo::yolov10n::kInputWidth, mtkdemo::yolov10n::kInputHeight,
            mtkdemo::yolov10n::kInt8InputQuant.scale, mtkdemo::yolov10n::kInt8InputQuant.zero_point,
            &letterbox);
        input_bytes = ToBytes(input_tensor);
      } else {
        const std::vector<float> input_tensor = mtkdemo::PreprocessToFloatRgbNchw(
            image, mtkdemo::yolov10n::kInputWidth, mtkdemo::yolov10n::kInputHeight, &letterbox);
        input_bytes = ToBytes(input_tensor);
      }
      ...
      const std::vector<std::vector<uint8_t>> bbox_outputs = {outputs[0], outputs[1], outputs[2]};
      const std::vector<std::vector<uint8_t>> cls_outputs = {outputs[3], outputs[4], outputs[5]};
      std::vector<mtkdemo::Detection> detections;
      if (use_int8_io) {
        std::vector<mtkdemo::YoloV10QuantParam> bbox_q;
        for (const auto& q : mtkdemo::yolov10n::kInt8BboxQuant) {
          bbox_q.push_back(mtkdemo::YoloV10QuantParam{q.scale, q.zero_point});
        }
        std::vector<mtkdemo::YoloV10QuantParam> cls_q;
        for (const auto& q : mtkdemo::yolov10n::kInt8ClsQuant) {
          cls_q.push_back(mtkdemo::YoloV10QuantParam{q.scale, q.zero_point});
        }
        detections = mtkdemo::DecodeYoloV10MultiOutputInt8(
            bbox_outputs, bbox_q, cls_outputs, cls_q, letterbox, image.width, image.height,
            mtkdemo::yolov10n::kConfidenceThreshold, mtkdemo::yolov10n::kIouThreshold,
            mtkdemo::yolov10n::kClassCount, mtkdemo::yolov10n::kRegMax,
            mtkdemo::yolov10n::kInputWidth);
      } else {
        detections = mtkdemo::DecodeYoloV10MultiOutput(
            bbox_outputs, cls_outputs, letterbox, image.width, image.height,
            mtkdemo::yolov10n::kConfidenceThreshold, mtkdemo::yolov10n::kIouThreshold,
            mtkdemo::yolov10n::kClassCount, mtkdemo::yolov10n::kRegMax, mtkdemo::yolov10n::kInputWidth);
      }
    ...
    std::cerr << "yolov10n_demo failed: " << ex.what() << "\n";
    return 1;
  }
}

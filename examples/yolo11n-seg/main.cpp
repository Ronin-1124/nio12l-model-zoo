...
#include "examples/yolo11n-seg/yolo11n-seg_config.h"
...
struct Options {
  ...
  std::string output_dir = mtkdemo::yolo11nseg::kDefaultOutputDir;
  size_t threads = 1;
  size_t backlog = 1;
...
Options ParseArgs(int argc, char** argv) {
  ...
  if (!options.model_overridden) {
    options.model_path = options.use_fp32 ? mtkdemo::yolo11nseg::kDefaultFp32ModelPath
                                          : mtkdemo::yolo11nseg::kDefaultInt8ModelPath;
  }
  if (options.image_paths.empty()) {
    options.image_paths.assign(mtkdemo::yolo11nseg::kDefaultImagePaths.begin(),
                               mtkdemo::yolo11nseg::kDefaultImagePaths.end());
  }
  return options;
}
...
int main(int argc, char** argv) {
  try {
    ...
    if (runtime.InputCount() != 1 || runtime.OutputCount() != 10) {
      throw std::runtime_error("Current demo expects one-input ten-output YOLO11n-seg model");
    }
    const size_t input_size = runtime.InputSize(0);
    if (input_size == static_cast<size_t>(mtkdemo::yolo11nseg::kInputWidth) *
                          static_cast<size_t>(mtkdemo::yolo11nseg::kInputHeight) * 3U) {
      use_int8_io = true;
    } else if (input_size == static_cast<size_t>(mtkdemo::yolo11nseg::kInputWidth) *
                                 static_cast<size_t>(mtkdemo::yolo11nseg::kInputHeight) * 3U *
                                 sizeof(float)) {
      use_int8_io = false;
    } else {
      throw std::runtime_error("Unsupported model input size for YOLO11n-seg demo");
    }
    ...
    for (const std::string& image_path : options.image_paths) {
      ...
      if (use_int8_io) {
        const auto input = mtkdemo::PreprocessToInt8RgbNchw(
            image, mtkdemo::yolo11nseg::kInputWidth, mtkdemo::yolo11nseg::kInputHeight,
            mtkdemo::yolo11nseg::kInt8InputQuant.scale, mtkdemo::yolo11nseg::kInt8InputQuant.zero_point,
            &letterbox);
        input_bytes = ToBytes(input);
      } else {
        const auto input = mtkdemo::PreprocessToFloatRgbNchw(
            image, mtkdemo::yolo11nseg::kInputWidth, mtkdemo::yolo11nseg::kInputHeight, &letterbox);
        input_bytes = ToBytes(input);
      }
      ...
      if (use_int8_io) {
        ...
        for (const auto& q : mtkdemo::yolo11nseg::kInt8CoeffQuant)
          coeff_q.push_back({q.scale, q.zero_point});
        for (const auto& q : mtkdemo::yolo11nseg::kInt8ClsQuant)
          cls_q.push_back({q.scale, q.zero_point});
        for (const auto& q : mtkdemo::yolo11nseg::kInt8BboxQuant)
        ...
        instances = mtkdemo::DecodeYoloV8SegMultiOutputInt8(
            ...
            {mtkdemo::yolo11nseg::kInt8ProtoQuant.scale, mtkdemo::yolo11nseg::kInt8ProtoQuant.zero_point},
            letterbox, image.width, image.height, mtkdemo::yolo11nseg::kConfidenceThreshold,
            mtkdemo::yolo11nseg::kIouThreshold, mtkdemo::yolo11nseg::kMaskThreshold,
            mtkdemo::yolo11nseg::kClassCount, mtkdemo::yolo11nseg::kRegMax, mtkdemo::yolo11nseg::kInputWidth,
            mtkdemo::yolo11nseg::kProtoHeight, mtkdemo::yolo11nseg::kProtoWidth);
      } else {
        instances = mtkdemo::DecodeYoloV8SegMultiOutput(
            ...
            mtkdemo::yolo11nseg::kConfidenceThreshold, mtkdemo::yolo11nseg::kIouThreshold,
            mtkdemo::yolo11nseg::kMaskThreshold, mtkdemo::yolo11nseg::kClassCount,
            mtkdemo::yolo11nseg::kRegMax, mtkdemo::yolo11nseg::kInputWidth, mtkdemo::yolo11nseg::kProtoHeight,
            mtkdemo::yolo11nseg::kProtoWidth);
      }
    ...
    std::cerr << "yolo11n-seg_demo failed: " << ex.what() << "\n";
    return 1;
  }
}

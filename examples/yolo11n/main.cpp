...
#include "common/cpp/yolo11_postprocess.h"
#include "examples/yolo11n/yolo11n_config.h"
...
struct Options {
  ...
  std::string output_dir = mtkdemo::yolo11n::kDefaultOutputDir;
  size_t threads = 1;
  size_t backlog = 1;
...
Options ParseArgs(int argc, char** argv) {
  ...
  if (!options.model_overridden) {
    options.model_path = options.use_fp32 ? mtkdemo::yolo11n::kDefaultFp32ModelPath
                                          : mtkdemo::yolo11n::kDefaultInt8ModelPath;
  }
  if (options.image_paths.empty()) {
    options.image_paths.assign(mtkdemo::yolo11n::kDefaultImagePaths.begin(),
                               mtkdemo::yolo11n::kDefaultImagePaths.end());
  }
  return options;
}
...
int main(int argc, char** argv) {
  try {
    ...
    if (runtime.InputCount() != 1 || runtime.OutputCount() != 6) {
      throw std::runtime_error("Current demo expects one-input six-output YOLO11n model");
    }

    const size_t input_size = runtime.InputSize(0);
    if (input_size == static_cast<size_t>(mtkdemo::yolo11n::kInputWidth) *
                          static_cast<size_t>(mtkdemo::yolo11n::kInputHeight) * 3U) {
      use_int8_io = true;
    } else if (input_size == static_cast<size_t>(mtkdemo::yolo11n::kInputWidth) *
                                 static_cast<size_t>(mtkdemo::yolo11n::kInputHeight) * 3U *
                                 sizeof(float)) {
      use_int8_io = false;
    } else {
      throw std::runtime_error("Unsupported model input size for YOLO11n demo: " +
    ...
    for (const std::string& image_path : options.image_paths) {
      ...
      if (use_int8_io) {
        const std::vector<int8_t> input_tensor = mtkdemo::PreprocessToInt8RgbNchw(
            image, mtkdemo::yolo11n::kInputWidth, mtkdemo::yolo11n::kInputHeight,
            mtkdemo::yolo11n::kInt8InputQuant.scale, mtkdemo::yolo11n::kInt8InputQuant.zero_point,
            &letterbox);
        input_bytes = ToBytes(input_tensor);
      } else {
        const std::vector<float> input_tensor = mtkdemo::PreprocessToFloatRgbNchw(
            image, mtkdemo::yolo11n::kInputWidth, mtkdemo::yolo11n::kInputHeight, &letterbox);
        input_bytes = ToBytes(input_tensor);
      }
      ...
      if (use_int8_io) {
        std::vector<mtkdemo::Yolo11QuantParam> bbox_q;
        bbox_q.reserve(mtkdemo::yolo11n::kInt8BboxQuant.size());
        for (const auto& q : mtkdemo::yolo11n::kInt8BboxQuant) {
          bbox_q.push_back(mtkdemo::Yolo11QuantParam{q.scale, q.zero_point});
        }
        std::vector<mtkdemo::Yolo11QuantParam> cls_q;
        cls_q.reserve(mtkdemo::yolo11n::kInt8ClsQuant.size());
        for (const auto& q : mtkdemo::yolo11n::kInt8ClsQuant) {
          cls_q.push_back(mtkdemo::Yolo11QuantParam{q.scale, q.zero_point});
        }
        detections = mtkdemo::DecodeYolo11MultiOutputInt8(
            bbox_outputs, bbox_q, cls_outputs, cls_q, letterbox, image.width, image.height,
            mtkdemo::yolo11n::kConfidenceThreshold, mtkdemo::yolo11n::kIouThreshold,
            mtkdemo::yolo11n::kClassCount, mtkdemo::yolo11n::kRegMax, mtkdemo::yolo11n::kInputWidth);
      } else {
        detections = mtkdemo::DecodeYolo11MultiOutput(
    ...
    std::cerr << "yolo11n_demo failed: " << ex.what() << "\n";
    return 1;
  }
}

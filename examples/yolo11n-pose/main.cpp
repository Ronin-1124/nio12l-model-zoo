...
#include "examples/yolo11n-pose/yolo11n-pose_config.h"
...
struct Options {
  ...
  std::string output_dir = mtkdemo::yolo11npose::kDefaultOutputDir;
  size_t threads = 1;
  size_t backlog = 1;
...
Options ParseArgs(int argc, char** argv) {
  ...
  if (!options.model_overridden) {
    options.model_path = options.use_fp32 ? mtkdemo::yolo11npose::kDefaultFp32ModelPath
                                          : mtkdemo::yolo11npose::kDefaultInt8ModelPath;
  }
  if (options.image_paths.empty()) {
    options.image_paths.assign(mtkdemo::yolo11npose::kDefaultImagePaths.begin(),
                               mtkdemo::yolo11npose::kDefaultImagePaths.end());
  }
  return options;
}
...
void DrawPose(mtkdemo::Image* image, const std::vector<mtkdemo::PoseInstance>& poses, float kpt_thr) {
  ...
  for (const auto& p : poses) {
    ...
    if (kp.size() < static_cast<size_t>(mtkdemo::yolo11npose::kKeypointCount) * 3U) continue;
    ...
    for (int i = 0; i < mtkdemo::yolo11npose::kKeypointCount; ++i) {
...
int main(int argc, char** argv) {
  try {
    ...
    if (runtime.InputCount() != 1 || runtime.OutputCount() != 9) {
      throw std::runtime_error("Current demo expects one-input nine-output YOLO11n-pose model");
    }
    const size_t input_size = runtime.InputSize(0);
    if (input_size == static_cast<size_t>(mtkdemo::yolo11npose::kInputWidth) *
                          static_cast<size_t>(mtkdemo::yolo11npose::kInputHeight) * 3U) {
      use_int8_io = true;
    } else if (input_size == static_cast<size_t>(mtkdemo::yolo11npose::kInputWidth) *
                                 static_cast<size_t>(mtkdemo::yolo11npose::kInputHeight) * 3U *
                                 sizeof(float)) {
      use_int8_io = false;
    } else {
      throw std::runtime_error("Unsupported model input size for YOLO11n-pose demo");
    }
    ...
    for (const std::string& image_path : options.image_paths) {
      ...
      if (use_int8_io) {
        const auto input = mtkdemo::PreprocessToInt8RgbNchw(
            image, mtkdemo::yolo11npose::kInputWidth, mtkdemo::yolo11npose::kInputHeight,
            mtkdemo::yolo11npose::kInt8InputQuant.scale, mtkdemo::yolo11npose::kInt8InputQuant.zero_point,
            &letterbox);
        input_bytes = ToBytes(input);
      } else {
        const auto input = mtkdemo::PreprocessToFloatRgbNchw(
            image, mtkdemo::yolo11npose::kInputWidth, mtkdemo::yolo11npose::kInputHeight, &letterbox);
        input_bytes = ToBytes(input);
      }
      ...
      if (use_int8_io) {
        ...
        for (const auto& q : mtkdemo::yolo11npose::kInt8KptQuant) kpt_q.push_back({q.scale, q.zero_point});
        for (const auto& q : mtkdemo::yolo11npose::kInt8ScoreQuant)
          score_q.push_back({q.scale, q.zero_point});
        for (const auto& q : mtkdemo::yolo11npose::kInt8BboxQuant) bbox_q.push_back({q.scale, q.zero_point});
        ...
        poses = mtkdemo::DecodeYoloV8PoseMultiOutputInt8(
            ...
            image.height, mtkdemo::yolo11npose::kConfidenceThreshold, mtkdemo::yolo11npose::kIouThreshold,
            mtkdemo::yolo11npose::kKeypointCount, mtkdemo::yolo11npose::kRegMax,
            mtkdemo::yolo11npose::kInputWidth);
      } else {
        poses = mtkdemo::DecodeYoloV8PoseMultiOutput(
            kpt_outputs, score_outputs, bbox_outputs, letterbox, image.width, image.height,
            mtkdemo::yolo11npose::kConfidenceThreshold, mtkdemo::yolo11npose::kIouThreshold,
            mtkdemo::yolo11npose::kKeypointCount, mtkdemo::yolo11npose::kRegMax,
            mtkdemo::yolo11npose::kInputWidth);
      }
      ...
      DrawPose(&vis, poses, mtkdemo::yolo11npose::kKeypointThreshold);
    ...
    std::cerr << "yolo11n-pose_demo failed: " << ex.what() << "\n";
    return 1;
  }
}

...
#include "examples/yolo26n-obb/yolo26n-obb_config.h"
...
struct Options {
  ...
  std::string output_dir = mtkdemo::yolo26nobb::kDefaultOutputDir;
  size_t threads = 1;
  size_t backlog = 1;
...
Options ParseArgs(int argc, char** argv) {
  ...
  if (!options.model_overridden) {
    options.model_path = options.use_fp32 ? mtkdemo::yolo26nobb::kDefaultFp32ModelPath
                                          : mtkdemo::yolo26nobb::kDefaultInt8ModelPath;
  }
  if (options.image_paths.empty()) {
    options.image_paths.assign(mtkdemo::yolo26nobb::kDefaultImagePaths.begin(),
                               mtkdemo::yolo26nobb::kDefaultImagePaths.end());
  }
  return options;
}
...
int main(int argc, char** argv) {
  try {
    ...
    if (runtime.InputCount() != 1 || runtime.OutputCount() != 9) {
      throw std::runtime_error("Current demo expects one-input nine-output YOLO26n-OBB model");
    }
    const size_t input_size = runtime.InputSize(0);
    if (input_size == static_cast<size_t>(mtkdemo::yolo26nobb::kInputWidth) *
                          static_cast<size_t>(mtkdemo::yolo26nobb::kInputHeight) * 3U) {
      use_int8_io = true;
    } else if (input_size == static_cast<size_t>(mtkdemo::yolo26nobb::kInputWidth) *
                                 static_cast<size_t>(mtkdemo::yolo26nobb::kInputHeight) * 3U *
                                 sizeof(float)) {
      use_int8_io = false;
    } else {
      throw std::runtime_error("Unsupported model input size for YOLO26n-OBB demo");
    }
    ...
    for (const std::string& image_path : options.image_paths) {
      ...
      if (use_int8_io) {
        const auto input = mtkdemo::PreprocessToInt8RgbNchw(
            image, mtkdemo::yolo26nobb::kInputWidth, mtkdemo::yolo26nobb::kInputHeight,
            mtkdemo::yolo26nobb::kInt8InputQuant.scale, mtkdemo::yolo26nobb::kInt8InputQuant.zero_point,
            &letterbox);
        input_bytes = ToBytes(input);
      } else {
        const auto input = mtkdemo::PreprocessToFloatRgbNchw(
            image, mtkdemo::yolo26nobb::kInputWidth, mtkdemo::yolo26nobb::kInputHeight, &letterbox);
        input_bytes = ToBytes(input);
      }
      ...
      if (use_int8_io) {
        std::vector<mtkdemo::YoloObbQuantParam> angle_q, bbox_q, cls_q;
        for (const auto& q : mtkdemo::yolo26nobb::kInt8AngleQuant) angle_q.push_back({q.scale, q.zero_point});
        for (const auto& q : mtkdemo::yolo26nobb::kInt8BboxQuant) bbox_q.push_back({q.scale, q.zero_point});
        for (const auto& q : mtkdemo::yolo26nobb::kInt8ClsQuant) cls_q.push_back({q.scale, q.zero_point});
        detections = mtkdemo::DecodeYolo26ObbMultiOutputInt8(
            angle_outputs, angle_q, bbox_outputs, bbox_q, cls_outputs, cls_q, letterbox, image.width,
            image.height, mtkdemo::yolo26nobb::kConfidenceThreshold,
            mtkdemo::yolo26nobb::kIouThreshold, mtkdemo::yolo26nobb::kClassCount,
            mtkdemo::yolo26nobb::kInputWidth);
      } else {
        detections = mtkdemo::DecodeYolo26ObbMultiOutput(
            angle_outputs, bbox_outputs, cls_outputs, letterbox, image.width, image.height,
            mtkdemo::yolo26nobb::kConfidenceThreshold, mtkdemo::yolo26nobb::kIouThreshold,
            mtkdemo::yolo26nobb::kClassCount, mtkdemo::yolo26nobb::kInputWidth);
      }
    ...
    std::cerr << "yolo26n-obb_demo failed: " << ex.what() << "\n";
    return 1;
  }
}

...
#include "examples/yolov5s/yolov5s_config.h"
...
struct Options {
  ...
  std::string output_dir = mtkdemo::yolov5s::kDefaultOutputDir;
  size_t threads = 1;
  size_t backlog = 1;
...
Options ParseArgs(int argc, char** argv) {
  ...
  if (!options.model_overridden) {
    options.model_path = options.use_fp32 ? mtkdemo::yolov5s::kDefaultFp32ModelPath
                                          : mtkdemo::yolov5s::kDefaultInt8ModelPath;
  }
  if (options.image_paths.empty()) {
    options.image_paths.assign(mtkdemo::yolov5s::kDefaultImagePaths.begin(),
                               mtkdemo::yolov5s::kDefaultImagePaths.end());
  }
...
int main(int argc, char** argv) {
  try {
    ...
    if (runtime.InputCount() != 1) {
      throw std::runtime_error("Current demo expects a single-input YOLOv5s model");
    }
    const size_t input_size = runtime.InputSize(0);
    if (input_size == static_cast<size_t>(mtkdemo::yolov5s::kInputWidth) *
                          static_cast<size_t>(mtkdemo::yolov5s::kInputHeight) * 3U) {
      use_int8_io = true;
    } else if (input_size == static_cast<size_t>(mtkdemo::yolov5s::kInputWidth) *
                                 static_cast<size_t>(mtkdemo::yolov5s::kInputHeight) * 3U *
                                 sizeof(float)) {
      use_int8_io = false;
    } else {
      throw std::runtime_error("Unsupported model input size for YOLOv5s demo: " +
    ...
    std::vector<mtkdemo::TensorQuantParam> int8_output_quant;
    int8_output_quant.reserve(mtkdemo::yolov5s::kInt8OutputQuant.size());
    for (const auto& q : mtkdemo::yolov5s::kInt8OutputQuant) {
      int8_output_quant.push_back(mtkdemo::TensorQuantParam{q.scale, q.zero_point});
    }
    ...
    for (const std::string& image_path : options.image_paths) {
      ...
      if (use_int8_io) {
        const std::vector<int8_t> input_tensor = mtkdemo::PreprocessToInt8RgbNchw(
            image, mtkdemo::yolov5s::kInputWidth, mtkdemo::yolov5s::kInputHeight,
            mtkdemo::yolov5s::kInt8InputQuant.scale, mtkdemo::yolov5s::kInt8InputQuant.zero_point,
            &letterbox);
        input_bytes = ToBytes(input_tensor);
      } else {
        const std::vector<float> input_tensor = mtkdemo::PreprocessToFloatRgbNchw(
            image, mtkdemo::yolov5s::kInputWidth, mtkdemo::yolov5s::kInputHeight, &letterbox);
        input_bytes = ToBytes(input_tensor);
      }
      ...
      if (use_int8_io) {
        detections = mtkdemo::DecodeYoloV5MultiOutputInt8(
            outputs, int8_output_quant, letterbox, image.width, image.height,
            mtkdemo::yolov5s::kConfidenceThreshold, mtkdemo::yolov5s::kIouThreshold,
            mtkdemo::yolov5s::kClassCount);
      } else {
        detections = mtkdemo::DecodeYoloV5MultiOutput(
    ...
    std::cerr << "yolov5s_demo failed: " << ex.what() << "\n";
    return 1;
  }
}

...
#include "examples/yolo12n/yolo12n_config.h"
...
struct Options {
  ...
  std::string output_dir = mtkdemo::yolo12n::kDefaultOutputDir;
  size_t threads = 1;
  size_t backlog = 1;
...
Options ParseArgs(int argc, char** argv) {
  ...
  if (!options.model_overridden) {
    options.model_path = options.use_fp32 ? mtkdemo::yolo12n::kDefaultFp32ModelPath
                                          : mtkdemo::yolo12n::kDefaultInt8ModelPath;
  }
  if (options.image_paths.empty()) {
    options.image_paths.assign(mtkdemo::yolo12n::kDefaultImagePaths.begin(),
                               mtkdemo::yolo12n::kDefaultImagePaths.end());
  }
  return options;
}
...
int main(int argc, char** argv) {
  try {
    ...
    if (runtime.InputCount() != 1 || runtime.OutputCount() != 6) {
      throw std::runtime_error("Current demo expects one-input six-output YOLO12n model");
    }
    const size_t input_size = runtime.InputSize(0);
    if (input_size == static_cast<size_t>(mtkdemo::yolo12n::kInputWidth) *
                          static_cast<size_t>(mtkdemo::yolo12n::kInputHeight) * 3U) {
      use_int8_io = true;
    } else if (input_size == static_cast<size_t>(mtkdemo::yolo12n::kInputWidth) *
                                 static_cast<size_t>(mtkdemo::yolo12n::kInputHeight) * 3U *
                                 sizeof(float)) {
      use_int8_io = false;
    } else {
      throw std::runtime_error("Unsupported model input size for YOLO12n demo: " +
    ...
    for (const std::string& image_path : options.image_paths) {
      ...
      if (use_int8_io) {
        const std::vector<int8_t> input_tensor = mtkdemo::PreprocessToInt8RgbNchw(
            image, mtkdemo::yolo12n::kInputWidth, mtkdemo::yolo12n::kInputHeight,
            mtkdemo::yolo12n::kInt8InputQuant.scale, mtkdemo::yolo12n::kInt8InputQuant.zero_point,
            &letterbox);
        input_bytes = ToBytes(input_tensor);
      } else {
        const std::vector<float> input_tensor = mtkdemo::PreprocessToFloatRgbNchw(
            image, mtkdemo::yolo12n::kInputWidth, mtkdemo::yolo12n::kInputHeight, &letterbox);
        input_bytes = ToBytes(input_tensor);
      }
      ...
      if (use_int8_io) {
        std::vector<mtkdemo::Yolo12QuantParam> bbox_q;
        for (const auto& q : mtkdemo::yolo12n::kInt8BboxQuant) {
          bbox_q.push_back(mtkdemo::Yolo12QuantParam{q.scale, q.zero_point});
        }
        std::vector<mtkdemo::Yolo12QuantParam> cls_q;
        for (const auto& q : mtkdemo::yolo12n::kInt8ClsQuant) {
          cls_q.push_back(mtkdemo::Yolo12QuantParam{q.scale, q.zero_point});
        }
        detections = mtkdemo::DecodeYolo12MultiOutputInt8(
            bbox_outputs, bbox_q, cls_outputs, cls_q, letterbox, image.width, image.height,
            mtkdemo::yolo12n::kConfidenceThreshold, mtkdemo::yolo12n::kIouThreshold,
            mtkdemo::yolo12n::kClassCount, mtkdemo::yolo12n::kRegMax, mtkdemo::yolo12n::kInputWidth);
      } else {
        detections = mtkdemo::DecodeYolo12MultiOutput(
    ...
    std::cerr << "yolo12n_demo failed: " << ex.what() << "\n";
    return 1;
  }
}

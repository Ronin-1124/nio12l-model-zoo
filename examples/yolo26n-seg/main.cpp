...
#include "examples/yolo26n-seg/yolo26n-seg_config.h"
...
struct Options {
  ...
  std::string output_dir = mtkdemo::yolo26nseg::kDefaultOutputDir;
  size_t threads = 1;
  size_t backlog = 1;
...
Options ParseArgs(int argc, char** argv) {
  ...
  if (!options.model_overridden) {
    options.model_path = options.use_fp32 ? mtkdemo::yolo26nseg::kDefaultFp32ModelPath
                                          : mtkdemo::yolo26nseg::kDefaultInt8ModelPath;
  }
  if (options.image_paths.empty()) {
    options.image_paths.assign(mtkdemo::yolo26nseg::kDefaultImagePaths.begin(),
                               mtkdemo::yolo26nseg::kDefaultImagePaths.end());
  }
  return options;
}
...
int main(int argc, char** argv) {
  try {
    ...
    if (runtime.InputCount() != 1 || runtime.OutputCount() != 10) {
      throw std::runtime_error("Current demo expects one-input ten-output YOLO26n-seg model");
    }
    const size_t input_size = runtime.InputSize(0);
    if (input_size == static_cast<size_t>(mtkdemo::yolo26nseg::kInputWidth) *
                          static_cast<size_t>(mtkdemo::yolo26nseg::kInputHeight) * 3U) {
      use_int8_io = true;
    } else if (input_size == static_cast<size_t>(mtkdemo::yolo26nseg::kInputWidth) *
                                 static_cast<size_t>(mtkdemo::yolo26nseg::kInputHeight) * 3U *
                                 sizeof(float)) {
      use_int8_io = false;
    } else {
      throw std::runtime_error("Unsupported model input size for YOLO26n-seg demo");
    }
    ...
    for (const std::string& image_path : options.image_paths) {
      ...
      if (use_int8_io) {
        const auto input = mtkdemo::PreprocessToInt8RgbNchw(
            image, mtkdemo::yolo26nseg::kInputWidth, mtkdemo::yolo26nseg::kInputHeight,
            mtkdemo::yolo26nseg::kInt8InputQuant.scale, mtkdemo::yolo26nseg::kInt8InputQuant.zero_point,
            &letterbox);
        input_bytes = ToBytes(input);
      } else {
        const auto input = mtkdemo::PreprocessToFloatRgbNchw(
            image, mtkdemo::yolo26nseg::kInputWidth, mtkdemo::yolo26nseg::kInputHeight, &letterbox);
        input_bytes = ToBytes(input);
      }
      ...
      const std::vector<std::vector<uint8_t>> cls_outputs = {outputs[6], outputs[7], outputs[8]};
      ...
      if (use_int8_io) {
        ...
        for (const auto& q : mtkdemo::yolo26nseg::kInt8CoeffQuant)
          coeff_q.push_back({q.scale, q.zero_point});
        for (const auto& q : mtkdemo::yolo26nseg::kInt8BboxQuant)
          bbox_q.push_back({q.scale, q.zero_point});
        for (const auto& q : mtkdemo::yolo26nseg::kInt8ClsQuant)
          cls_q.push_back({q.scale, q.zero_point});
        instances = mtkdemo::DecodeYolo26SegMultiOutputInt8(
            coeff_outputs, coeff_q, bbox_outputs, bbox_q, cls_outputs, cls_q, proto_output,
            {mtkdemo::yolo26nseg::kInt8ProtoQuant.scale, mtkdemo::yolo26nseg::kInt8ProtoQuant.zero_point},
            letterbox, image.width, image.height, mtkdemo::yolo26nseg::kConfidenceThreshold,
            mtkdemo::yolo26nseg::kIouThreshold, mtkdemo::yolo26nseg::kMaskThreshold,
            mtkdemo::yolo26nseg::kClassCount, mtkdemo::yolo26nseg::kInputWidth,
            mtkdemo::yolo26nseg::kProtoHeight, mtkdemo::yolo26nseg::kProtoWidth);
      } else {
        instances = mtkdemo::DecodeYolo26SegMultiOutput(
            coeff_outputs, bbox_outputs, cls_outputs, proto_output, letterbox, image.width, image.height,
            mtkdemo::yolo26nseg::kConfidenceThreshold, mtkdemo::yolo26nseg::kIouThreshold,
            mtkdemo::yolo26nseg::kMaskThreshold, mtkdemo::yolo26nseg::kClassCount,
            mtkdemo::yolo26nseg::kInputWidth, mtkdemo::yolo26nseg::kProtoHeight,
            mtkdemo::yolo26nseg::kProtoWidth);
      }
    ...
    std::cerr << "yolo26n-seg_demo failed: " << ex.what() << "\n";
    return 1;
  }
}

...
#include "examples/yolov8n-pose/yolov8n-pose_config.h"
...
struct Options {
  ...
  std::string output_dir = mtkdemo::yolov8npose::kDefaultOutputDir;
  size_t threads = 1;
  size_t backlog = 1;
...
Options ParseArgs(int argc, char** argv) {
  ...
  if (!options.model_overridden) {
    options.model_path = options.use_fp32 ? mtkdemo::yolov8npose::kDefaultFp32ModelPath
                                          : mtkdemo::yolov8npose::kDefaultInt8ModelPath;
  }
  if (options.image_paths.empty()) {
    options.image_paths.assign(mtkdemo::yolov8npose::kDefaultImagePaths.begin(),
                               mtkdemo::yolov8npose::kDefaultImagePaths.end());
  }
  return options;
}
...
void DrawPose(mtkdemo::Image* image, const std::vector<mtkdemo::PoseInstance>& poses, float kpt_thr) {
  ...
  for (const auto& p : poses) {
    ...
    if (kp.size() < static_cast<size_t>(mtkdemo::yolov8npose::kKeypointCount) * 3U) continue;
    ...
    for (const auto& e : kEdges) {
      ...
      const int ax = static_cast<int>(std::lround(kp[static_cast<size_t>(a) * 3U + 0U]));
      const int ay = static_cast<int>(std::lround(kp[static_cast<size_t>(a) * 3U + 1U]));
      const int bx = static_cast<int>(std::lround(kp[static_cast<size_t>(b) * 3U + 0U]));
      const int by = static_cast<int>(std::lround(kp[static_cast<size_t>(b) * 3U + 1U]));
      DrawLine(image, ax, ay, bx, by, 0, 255, 255);
    }

    // Draw keypoints.
    for (int i = 0; i < mtkdemo::yolov8npose::kKeypointCount; ++i) {
      const float x = kp[static_cast<size_t>(i) * 3U + 0U];
      const float y = kp[static_cast<size_t>(i) * 3U + 1U];
      const float s = kp[static_cast<size_t>(i) * 3U + 2U];
      if (s < kpt_thr) continue;
      DrawCircle(image, static_cast<int>(std::lround(x)), static_cast<int>(std::lround(y)), 2, 0, 255, 0);
    }
  }
}
...
int main(int argc, char** argv) {
  try {
    ...
    if (runtime.InputCount() != 1 || runtime.OutputCount() != 9) {
      throw std::runtime_error("Current demo expects one-input nine-output YOLOv8n-pose model");
    }
    const size_t input_size = runtime.InputSize(0);
    if (input_size == static_cast<size_t>(mtkdemo::yolov8npose::kInputWidth) *
                          static_cast<size_t>(mtkdemo::yolov8npose::kInputHeight) * 3U) {
      use_int8_io = true;
    } else if (input_size == static_cast<size_t>(mtkdemo::yolov8npose::kInputWidth) *
                                 static_cast<size_t>(mtkdemo::yolov8npose::kInputHeight) * 3U *
                                 sizeof(float)) {
      use_int8_io = false;
    } else {
      throw std::runtime_error("Unsupported model input size for YOLOv8n-pose demo");
    }
    ...
    for (const std::string& image_path : options.image_paths) {
      ...
      if (use_int8_io) {
        const auto input = mtkdemo::PreprocessToInt8RgbNchw(
            image, mtkdemo::yolov8npose::kInputWidth, mtkdemo::yolov8npose::kInputHeight,
            mtkdemo::yolov8npose::kInt8InputQuant.scale, mtkdemo::yolov8npose::kInt8InputQuant.zero_point,
            &letterbox);
        input_bytes = ToBytes(input);
      } else {
        const auto input = mtkdemo::PreprocessToFloatRgbNchw(
            image, mtkdemo::yolov8npose::kInputWidth, mtkdemo::yolov8npose::kInputHeight, &letterbox);
        input_bytes = ToBytes(input);
      }
      ...
      const std::vector<std::vector<uint8_t>> score_outputs = {outputs[3], outputs[4], outputs[5]};
      const std::vector<std::vector<uint8_t>> bbox_outputs = {outputs[6], outputs[7], outputs[8]};

      std::vector<mtkdemo::PoseInstance> poses;
      if (use_int8_io) {
        std::vector<mtkdemo::YoloV8PoseQuantParam> kpt_q;
        std::vector<mtkdemo::YoloV8PoseQuantParam> score_q;
        std::vector<mtkdemo::YoloV8PoseQuantParam> bbox_q;
        for (const auto& q : mtkdemo::yolov8npose::kInt8KptQuant) kpt_q.push_back({q.scale, q.zero_point});
        for (const auto& q : mtkdemo::yolov8npose::kInt8ScoreQuant)
          score_q.push_back({q.scale, q.zero_point});
        for (const auto& q : mtkdemo::yolov8npose::kInt8BboxQuant) bbox_q.push_back({q.scale, q.zero_point});

        poses = mtkdemo::DecodeYoloV8PoseMultiOutputInt8(
            kpt_outputs, kpt_q, score_outputs, score_q, bbox_outputs, bbox_q, letterbox, image.width,
            image.height, mtkdemo::yolov8npose::kConfidenceThreshold, mtkdemo::yolov8npose::kIouThreshold,
            mtkdemo::yolov8npose::kKeypointCount, mtkdemo::yolov8npose::kRegMax,
            mtkdemo::yolov8npose::kInputWidth);
      } else {
        poses = mtkdemo::DecodeYoloV8PoseMultiOutput(
            kpt_outputs, score_outputs, bbox_outputs, letterbox, image.width, image.height,
            mtkdemo::yolov8npose::kConfidenceThreshold, mtkdemo::yolov8npose::kIouThreshold,
            mtkdemo::yolov8npose::kKeypointCount, mtkdemo::yolov8npose::kRegMax,
            mtkdemo::yolov8npose::kInputWidth);
      }

      std::vector<mtkdemo::Detection> dets;
      dets.reserve(poses.size());
      ...
      DrawPose(&vis, poses, mtkdemo::yolov8npose::kKeypointThreshold);
    ...
    std::cerr << "yolov8n-pose_demo failed: " << ex.what() << "\n";
    return 1;
  }
}

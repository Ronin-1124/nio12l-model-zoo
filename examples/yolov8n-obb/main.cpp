...
#include "examples/yolov8n-obb/yolov8n-obb_config.h"
...
struct Options {
  ...
  std::string output_dir = mtkdemo::yolov8nobb::kDefaultOutputDir;
  size_t threads = 1;
  size_t backlog = 1;
...
Options ParseArgs(int argc, char** argv) {
  ...
  if (!options.model_overridden) {
    options.model_path = options.use_fp32 ? mtkdemo::yolov8nobb::kDefaultFp32ModelPath
                                          : mtkdemo::yolov8nobb::kDefaultInt8ModelPath;
  }
  if (options.image_paths.empty()) {
    options.image_paths.assign(mtkdemo::yolov8nobb::kDefaultImagePaths.begin(),
                               mtkdemo::yolov8nobb::kDefaultImagePaths.end());
  }
  return options;
}
...
int main(int argc, char** argv) {
  try {
    ...
    if (runtime.InputCount() != 1 || runtime.OutputCount() != 9) {
      throw std::runtime_error("Current demo expects one-input nine-output YOLOv8n-OBB model");
    }
    const size_t input_size = runtime.InputSize(0);
    if (input_size == static_cast<size_t>(mtkdemo::yolov8nobb::kInputWidth) *
                          static_cast<size_t>(mtkdemo::yolov8nobb::kInputHeight) * 3U) {
      use_int8_io = true;
    } else if (input_size == static_cast<size_t>(mtkdemo::yolov8nobb::kInputWidth) *
                                 static_cast<size_t>(mtkdemo::yolov8nobb::kInputHeight) * 3U *
                                 sizeof(float)) {
      use_int8_io = false;
    } else {
      throw std::runtime_error("Unsupported model input size for YOLOv8n-OBB demo");
    }
    ...
    for (const std::string& image_path : options.image_paths) {
      ...
      if (use_int8_io) {
        const auto input = mtkdemo::PreprocessToInt8RgbNchw(
            image, mtkdemo::yolov8nobb::kInputWidth, mtkdemo::yolov8nobb::kInputHeight,
            mtkdemo::yolov8nobb::kInt8InputQuant.scale, mtkdemo::yolov8nobb::kInt8InputQuant.zero_point,
            &letterbox);
        input_bytes = ToBytes(input);
      } else {
        const auto input = mtkdemo::PreprocessToFloatRgbNchw(
            image, mtkdemo::yolov8nobb::kInputWidth, mtkdemo::yolov8nobb::kInputHeight, &letterbox);
        input_bytes = ToBytes(input);
      }
      ...
      if (use_int8_io) {
        ...
        for (const auto& q : mtkdemo::yolov8nobb::kInt8AngleQuant) angle_q.push_back({q.scale, q.zero_point});
        for (const auto& q : mtkdemo::yolov8nobb::kInt8ClsQuant) cls_q.push_back({q.scale, q.zero_point});
        for (const auto& q : mtkdemo::yolov8nobb::kInt8BboxQuant) bbox_q.push_back({q.scale, q.zero_point});
        detections = mtkdemo::DecodeYoloV8ObbMultiOutputInt8(
            ...
            image.height, mtkdemo::yolov8nobb::kConfidenceThreshold, mtkdemo::yolov8nobb::kIouThreshold,
            mtkdemo::yolov8nobb::kClassCount, mtkdemo::yolov8nobb::kRegMax, mtkdemo::yolov8nobb::kInputWidth);
      } else {
        detections = mtkdemo::DecodeYoloV8ObbMultiOutput(
            angle_outputs, cls_outputs, bbox_outputs, letterbox, image.width, image.height,
            mtkdemo::yolov8nobb::kConfidenceThreshold, mtkdemo::yolov8nobb::kIouThreshold,
            mtkdemo::yolov8nobb::kClassCount, mtkdemo::yolov8nobb::kRegMax, mtkdemo::yolov8nobb::kInputWidth);
      }
    ...
    std::cerr << "yolov8n-obb_demo failed: " << ex.what() << "\n";
    return 1;
  }
}
